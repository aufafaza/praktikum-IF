...
jarak (Pt x1 y1) (Pt x2 y2) = 
    let 
        jarak_x = x2 - x1
        jarak_y = y2 - y1
        sqr_x = jarak_x * jarak_x
        sqr_y = jarak_y * jarak_y
    in 
        sqrt(sqr_x + sqr_y)
...
jarak0 p = 
    let 
        jarak_x = absis p * absis p 
        jarak_y = ordinat p * ordinat p 

    in 
        sqrt(jarak_x + jarak_y)

-- movePoint: Float -> Float -> point -> point
-- {movePoint(dx,dy,P) menggeser titik P sejauh dx pada sumbu x dan dy pada sumbu y}
movePoint :: Float -> Float -> Point -> Point
movePoint dx dy (Pt x y) = 
    let new_x = x + dx
        new_y = y + dy 
    in 
        makePoint new_x new_y
