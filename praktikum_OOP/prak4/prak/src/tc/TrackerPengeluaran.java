public class TrackerPengeluaran {
	...
	public void setCatatan(int minggu, int kategori, int nominal, String deskripsi) {
		// TODO:
		// 1. Ubah parameter minggu dari indeks berbasis 1 menjadi indeks array berbasis
		// 0.
	...
	public int totalPerMinggu(int minggu) {
		...
		int total = 0;
		for (int i = 0; i < 3; i++) {
			if (matriks[minggu - 1][i] != null) {
				total += matriks[minggu - 1][i].getNominal();
			}
		}
	...
	public void cetakLaporan() {
		...
		// 2. Untuk setiap minggu (1 sampai 4), cetak nomor minggu di awal baris diikuti
		// dengan "\t".
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 3; j++) {
				if (matriks[i][j] != null) {
					System.out.print(matriks[i][j].getNominal() + " ("
							+ matriks[i][j].getDeskripsi() + ")\t");
				} else {
